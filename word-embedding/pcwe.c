...
char non_comp[MAX_STRING], char_init[MAX_STRING]; char save_pos_file[MAX_STRING];
struct vocab_word *vocab; struct vocab_pos_ *vocab_pos;
...
int *vocab_hash; int *pos_hash;
long long vocab_max_size = 100000, vocab_size = 0, dim = 100, character_size = 0;
long long pos_max_size = 100, vocab_pos_size = 0;
long long train_words = 0, word_count_actual = 0, iter = 5, file_size = 0;
real alpha = 0.025, starting_alpha, sample = 1e-3, nonpara = 1e-3, char_rate = 0.1, pos_rate = 0.1;
...
void ReadWord(char *word, char *pos0, FILE *fin) {
	int a = 0, b = 0, c = 1, ch;
	while (!feof(fin)) {
		ch = fgetc(fin);
		if (ch == 13) continue;
...
int GetPosHash(char *pos) {
	unsigned long long a , hash = 0;
	for (a = 0; a < strlen(pos); a++) hash = hash * 257 + pos[a];
	hash = hash % pos_hash_size;
	return hash;
...
int SearchPos(char *pos0) {
	unsigned int hash = GetPosHash(pos0);
	while(1) {
		if (pos_hash[hash] == -1) return -1;
		if(!strcmp(pos0, vocab_pos[pos_hash[hash]].pos)) return pos_hash[hash];
		hash = (hash + 1) % pos_hash_size;
	}
	return -1;
}

// Reads a word and returns its index in the vocabulary
void ReadWordIndex(FILE *fin, int *A) {
//	int *A = malloc(2*sizeof(int));
	A[0] = -1; A[1] = -1;
	char word[MAX_STRING];
	char pos0[MAX_STRING];
	ReadWord(word, pos0, fin);
	if (feof(fin)) return ;
	A[0] = SearchVocab(word);
	A[1] = SearchPos(pos0);
//	return A;
}
...
int AddPosToVocab(char *pos0) {
	unsigned int hash, length = strlen(pos0) + 1;
	vocab_pos[vocab_pos_size].pos = (char *)calloc(length, sizeof(char));
	strcpy(vocab_pos[vocab_pos_size].pos, pos0);
	vocab_pos_size++;
	hash = GetPosHash(pos0);
	while (pos_hash[hash] != -1) hash = (hash + 1) % pos_hash_size;
	pos_hash[hash] = vocab_pos_size - 1;
	return vocab_pos_size - 1;
}
...
void LearnVocabFromTrainFile() {
	char word[MAX_STRING]; char pos0[MAX_STRING];
	FILE *fin;
	long long a, i;
	for (a = 0; a < vocab_hash_size; a++) vocab_hash[a] = -1;
	for (a = 0; a < pos_hash_size; a++) pos_hash[a] = -1;
	fin = fopen(train_file, "rb");
	if (fin == NULL) {
		printf("ERROR: training data file not found!\n");
		exit(1);
	}
	vocab_size = 0;int k=0;
	AddWordToVocab((char *)"</s>", 0);
	AddPosToVocab((char *)"</s>");
	...
	while (1) {
		...
		if (vocab_size > vocab_hash_size * 0.7) ReduceVocab();
		i = SearchPos(pos0);//printf("search p\n");
		if (i == -1) {
			a = AddPosToVocab(pos0);//printf("add p \n");		
		}
	}
	SortVocab();
	if (debug_mode > 0) {
...
void InitNet() {
	...
	for (a = 0; a < vocab_size; a++) for (b = 0; b < dim; b++) {
		...
		if(syn0[a * dim + b] > 10 || syn0[a * dim + b] < -10) printf("%f ", syn0[a*dim+b]); 
		//printf("%f ", syn0[a*dim+b]);
	}
	if (cwe_type) {
	...
	a = posix_memalign((void **)&posv, 128, (long long) vocab_pos_size * dim * sizeof(real));
	if (posv == NULL) {printf("Memory allocation failed\n"); exit(1);}
	for(a = 0; a < (long long) vocab_pos_size * dim; a++) {
		next_random = next_random * (unsigned long long)25214903917 + 11;
		posv[a]  =  (((next_random & 0xFFFF) / (real)65536) - 0.5) / dim;

		//assert(!( posv[a] > 10 || posv[a]<-10));
	}
...
void *TrainModelThread(void *id) {
	...
	long long a, b, d, cw,  word, last_word, pos, last_pos, sentence_length = 0, sentence_position = 0, charv_id;
	long long word_count = 0, last_word_count = 0, sen[MAX_SENTENCE_LENGTH + 1], senPos[MAX_SENTENCE_LENGTH + 1];
	...
	real *neu1char = (real *)calloc(dim, sizeof(real));
	...
	while (1) {
		...
		//		printf("read %lld\n", (long long)id);
				ReadWordIndex(fi, A);
				word = A[0]; pos = A[1];
		//		printf("%d %d %lld\n",  word, pos, (long long)id);
				
				if (feof(fi)) break;
				if (word == -1 || pos == -1) continue;
				word_count++;
				if (word == 0 || pos == 0) break;
		...
		//printf("begin4 %lld\n", (long long)id);
			...
			for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				...
				if (cwe_type && vocab[last_word].character_size) {
					...
					for (c = 0; c < vocab[last_word].character_size; c++) {
						...
						for (d = 0; d < dim; d++){
							...
							//assert(!( neu1char[c] > 10 || neu1char[c]<-10));
						}
				...
				for(c = 0; c < dim; c++) {
					neu1char[c] += posv[c + last_pos * dim];
					//printf("neu1char %lld %lld %f %lld\n",(c+last_pos * dim), last_pos,  posv[c + last_pos * dim], (long long)id);
					//assert(!( neu1char[c] > 10 || neu1char[c]<-10));
					//assert(!( posv[c + last_pos * dim] > 10 ||posv[c + last_pos * dim] < -10));
				}
				for (d = 0; d < dim; d++) {
					neu1char[d] /= 3;
					//assert(!( neu1char[c] > 10 || neu1char[c]<-10));				
				...
				for (c = 0; c < dim; c++) {
					neu1[c] += neu1char[c];
					//printf("neu1 %f %f %lld\n", neu1[c],  neu1char[c], (long long)id);
					//if(neu1[c] > 10 || neu1[c] < -10) printf("%f %lld\n", neu1[c], (long long)id);
					//assert(!( neu1[c] > 10 || neu1[c]<-10));
					//assert(!( neu1char[c] > 10 || neu1char[c]<-10));				
				}
				cw++;
			}
			...
			//printf("hehe5_1_1 %d %lld\n", d, (long long)id);
					l2 = target * dim;
					f = 0;
					for (c = 0; c < dim; c++){
						//printf("%f %f %f %lld\n", neu1[c], syn1neg[c+l2], f, (long long)id);
						f += neu1[c] * syn1neg[c + l2];
					//	if(f != f){f=-MAX_EXP-1;
					}
			//printf("hehe5_1_1_1 %d %lld\n", d, (long long)id);
					if (f > MAX_EXP) g = (label - 1) * alpha;
					else if (f < -MAX_EXP) g = (label - 0) * alpha;
					else g = (label - expTable[(int)((f + MAX_EXP) * (EXP_TABLE_SIZE / MAX_EXP / 2))]) * alpha;
			//printf("hehe5_1_1_2 %d %lld\n", d, (long long)id);
					for (c = 0; c < dim; c++) {
						neu1e[c] += g * syn1neg[c + l2];
						//printf("%f %f \n", g, syn1neg[c+l2]);
					//	assert(!( neu1e[c] > 10 || neu1e[c]<-10));
					//	if(neu1e[c] > 10 || neu1e[c] < -10){
					//		printf("neu1e %f \n", neu1e[c]);
					//	}
					}
			//printf("hehe5_1_1_3 %d %lld\n", d, (long long)id);
					for (c = 0; c < dim; c++) { 
						syn1neg[c + l2] += g * neu1[c];
						//printf("syn1nega %f %lld\n", syn1neg[c+l2], (long long)id);
					
					//	assert(!( syn1neg[c+l2] > 10 || syn1neg[c+l2]<-10));
					//	if(syn1neg[c+l2] > 10 || syn1neg[c+l2] < -10){
					//		printf("syn1neg %f \n", syn1neg[c+l2]);
					//	}
					} 
			//printf("hehe5_1_2 %d %lld\n", d, (long long)id);
				}
			//printf("hehe5_2 %lld\n", (long long)id);
				// hidden -> in
				for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
			//printf("hehe5_2_1 %d %lld\n", a , (long long)id);
					...
					for (c = 0; c < dim; c++){
					//	assert(!( neu1e[c] > 10 || neu1e[c]<-10));
						syn0[c + last_word * dim] += neu1e[c];	
					//	assert(!( syn0[c + last_word * dim] > 10 || syn0[c + last_word*dim]<-10));
					}
					/*for (a = 0; a < vocab_size; a++) for (b = 0; b < dim; b++) {
						if(syn0[a * dim + b] > 10 || syn0[a * dim + b] < -10) {printf("%f ", syn0[a*dim+b]); exit(1); } 
					}*/
					for (c = 0; c < dim; c++) {
						posv[c + last_pos * dim] += neu1e[c] * pos_rate;
					//	assert(!( posv[c + last_pos * dim] > 10 || posv[c + last_pos * dim]<-10));						
					}
				}
			//printf("hehe5_3 %lld\n", (long long)id);
			...
			for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
				...
				for (c = 0; c < dim; c++) neu1[c] += posv[c + last_pos * dim];
				for (d = 0; d < dim; d++) neu1[d] /= 3;
				for (c = 0; c < dim; c++) neu1e[c] = 0;
				...
				for (c = 0; c < dim; c++) posv[c + last_pos * dim] += neu1e[c] * pos_rate;
		...
		sentence_position++;
		//printf("sss %d %d %lld\n", sentence_position, sentence_length, (long long)id);
...
void TrainModel() {
	...
	fclose(fo);
	fo = fopen(save_pos_file, "wb");
	for (a = 0; a < vocab_pos_size; a++) {
		fprintf(fo, "%s ", vocab_pos[a].pos);
		for (b = 0; b < dim; b++) fprintf(fo, "%lf ",posv[a * dim + b]);
		fprintf(fo, "\n");
	}
	fclose(fo);
...
int main(int argc, char **argv) {
	...
	if (argc == 1) {
		...
		printf("\t\tSet the factor <float> of learning rate for characters, default is    0.1 (0.0 for fixed character vectors)\n");
		printf("\t-pos-rate <float>\n");
		printf("\t\tSet the factor <float> of learning rate for pos, default is 0.1 \n");
		printf("\t-cwin <int>\n");
	...
	if ((i = ArgPos((char *)"-pos-rate", argc, argv)) > 0) pos_rate = atof(argv[i + 1]);
	if ((i = ArgPos((char *)"-save-pos-file", argc, argv)) > 0) strcpy(save_pos_file, argv[i + 1]);



	vocab = (struct vocab_word *)calloc(vocab_max_size, sizeof(struct vocab_word));
	vocab_hash = (int *)calloc(vocab_hash_size, sizeof(int));
	vocab_pos = (struct vocab_pos_ *)calloc(pos_max_size, sizeof(struct vocab_pos_));
	pos_hash = (int *)calloc(pos_hash_size, sizeof(int));
