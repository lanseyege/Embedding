...
real rp_sample = 0.1;
char *vocab_file, *file_head, *corpus_name;
...
HASHREC *hashsearch(HASHREC **ht, char *w, int line) {
    HASHREC	*htmp, *hprv;
...
int merge_files(int num) {
    ...
    for(i = 0; i < num; i++) {
        sprintf(filename,"./temp/%s_%04d.bin",file_head,i);
...
int get_cooccurrence() {
    ...
    int max_lengs = 1e6, t_lengs = 0, lines_num = 0;
    int flag, x, y, fidcounter = 1, sentence[max_lengs], end=0;
    long long a, b, j = 0, k, id, counter = 0, ind = 0, vocab_size, w1, w2, *lookup, *history;
    ...
    sprintf(filename,"./temp/%s_%04d.bin",file_head, fidcounter);
    ...
    while (1) {
        while (1) {
            ...
            if (flag == 1) {
                ...
                htmp = hashsearch(vocab_hash, str, lines_num);
                if (htmp == NULL) continue; // Skip out-of-vocabulary words
                //next_random = next_random * (unsigned long long)25214903917 + 11;
                //if (rp_sample < (next_random & 0xFFFF) / (real)65536) continue;
        ...
        if (end) {end = 0; break; }
        //if (lines_num > lines_size) break;
        if (t_lengs == 0) break;
        
        //sprintf(filename,"./temp/%s_%04d.bin",file_head, fidcounter);
        //foverflow = fopen(filename,"w");
//lines_num = 0;
        if(ind >= overflow_length - t_lengs  * t_lengs /2) {
            kk ++;
        ...
        for (a = 1; a < t_lengs; a++) {
            ...
            for (b = a; b >= ((a-window_size>0) ? a-window_size: 0); b--) {
                w1 = sentence[b];
                if (w1 < max_product/w2) {
                    bigram_table[lookup[w1-1] + w2 - 2] += 1.0/((real)(b-a+1));
                    if (symmetric > 0) bigram_table[lookup[w2-1] + w1 - 2] += 1.0/((real)(b-a+1));
                }
                else {
                    cr[ind].word1 = w1;
                    cr[ind].word2 = w2;
                    cr[ind].val = 1.0/((real)(b-a+1));
                    ind++;
                    if (symmetric > 0) {
                        cr[ind].word1 = w2;
                        cr[ind].word2 = w1;
            ...
            for (b = 0; b < t_lengs; b++) {
                if (b <= a && b >= ((a-window_size>0) ? a-window_size: 0)) continue;
                next_random = next_random * (unsigned long long)25214903917 + 11;
                if (rp_sample < (next_random & 0xFFFF) / (real)65536) continue;
                w1 = sentence[b];
                
                if (w1 < max_product/w2) {
                    bigram_table[lookup[w1-1] + w2 - 2] += 1.0/((real)(abs(b-a)+1));
                    if (symmetric > 0) bigram_table[lookup[w2-1] + w1 - 2] += 1.0/((real)(abs(b-a)+1));
                }
                else {
                    cr[ind].word1 = w1;
                    cr[ind].word2 = w2;
                    cr[ind].val = 1.0/((real)(abs(b-a)+1));
                    ind++;
                    if (symmetric > 0) {
                        cr[ind].word1 = w2;
                        cr[ind].word2 = w1;
    ...
    sprintf(filename,"./temp/%s_0000.bin",file_head);
    ...
    fprintf(stderr, "hehe--\n");
    fclose(foverflow);
    fprintf(stderr, "hehe1\n");
    free(cr);
    fprintf(stderr, "hehe2\n");
    free(lookup);
    fprintf(stderr, "hehe3\n");
    free(bigram_table);
    fprintf(stderr, "hehe4\n");
    free(vocab_hash);
    fprintf(stderr, "hehe5\n");
...
int main(int argc, char **argv) {
    ...
    corpus_name = malloc(sizeof(char) * MAX_STRING_LENGTH);
    ...
    if ((i = find_arg((char *)"-corpus-name", argc, argv)) > 0) strcpy(corpus_name, argv[i + 1]);
    if ((i = find_arg((char *)"-rp-sample", argc, argv)) > 0) rp_sample = atof(argv[i + 1]);
